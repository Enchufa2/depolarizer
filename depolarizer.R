depolarizer <- function(img_i, res_o=1000, out.color=c(0, 0, 0)) {
  ...
  if (length(out.color) == 3) {
    outside <- rowSums(sapply(pix_i, findInterval, c(1, dim_i)) != 1) > 0
  ...
  for (i in seq_along(cha_o)) {
    dim(cha_o[[i]]) <- dim(imager::channel(img_o, i))
    imager::channel(img_o, i) <- cha_o[[i]]
  }
  img_o
}
